...
class CrashWatchdog {
  ...
  async _attemptRestart() {
    ...
    this.phase = 'idle';
    this.lastError = restartError?.message || 'Palworld was still offline after the restart attempt.';
  ...
  async start({ adoptRunning = true } = {}) {
    ...
    if (running && adoptRunning && !this.desiredRunning) {
      ...
      this.log('watchdog', 'Crash recovery adopted the already-running Palworld server.');
    }
    ...
    if (!running && this.desiredRunning && this.settings.enabled) {
      this._scheduleRecovery('Palworld was expected to be online when PalSphere started.');
    }
  }
  ...
  async tick() {
    ...
    try {
      ...
      if (wasRunning && this.desiredRunning && this.settings.enabled && !updating) {
        ...
        this.lastError = null;
        this.phase = 'idle';
        this.log('crash', 'Palworld stopped unexpectedly; crash recovery was triggered.', {
          detectedAt: this.lastCrashAt,
        });
        this._scheduleRecovery('The Palworld server process exited unexpectedly.');
        return;
      }
      ...
      if (!this.settings.enabled) this.phase = 'disabled';
        ...
        this._scheduleRecovery('Palworld is offline but is expected to be online.');
      }
    } finally {
      this.checkInFlight = false;
    }
  ...
  updateSettings(settings) {
    ...
    if (!this.settings.enabled) this.phase = 'disabled';
      ...
      this.phase = 'idle';
      if (this.desiredRunning) this._scheduleRecovery('Crash recovery was enabled while Palworld was expected online.');
    }
